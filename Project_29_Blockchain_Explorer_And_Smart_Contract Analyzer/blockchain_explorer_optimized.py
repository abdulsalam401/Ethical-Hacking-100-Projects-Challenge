#!/usr/bin/env python3
"""
PROJECT #29 — Blockchain Explorer (Optimized)
"""
...
class Explorer:
    ...
    def get_transactions(self, address, max_blocks=100, max_txs=10):
        """Fetch transactions with limits"""
        print(f"\n[*] Fetching transactions for {address}")
        print(f"[*] Scanning last {max_blocks} blocks (max {max_txs} txs)")
        ...
        try:
            ...
            found = 0
            for block_num in range(latest, max(0, latest - max_blocks), -1):
                if found >= max_txs:
                    break
                ...
                try:
                    ...
                    for tx in block.transactions:
                        if tx['from'].lower() == address.lower() or (tx.get('to') and tx['to'].lower() == address.lower()):
                            ...
                            found += 1
                            print(f"[*] Found tx {found}: {tx['hash'].hex()[:16]}...")
                            
                except Exception as e:
                    # Skip blocks with errors
                    continue
            
            print(f"[+] Found {len(txs)} transactions ({self.request_count} API requests)")
            return txs
...
<p>API Requests: {self.request_count}</p>
</div>"""
...
<tr><td>Timestamp</td><td>Avoid block.timestamp</td></tr>
    ...
    def run(self, address=None, contract=None):
        ...
        if address:
            txs = self.get_transactions(address, max_blocks=50, max_txs=10)

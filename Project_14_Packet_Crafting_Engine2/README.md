# 🛠️ Interactive Scapy-Based Packet Crafting & Fuzzing Tool
...
## 📌 Project Overview
An interactive packet crafting utility. It offers fuzzing presets, such as sending packets with randomized flags, randomized payloads, or randomized destination ports to test the resilience of local network interfaces.

### ⚙️ Specifications & Tech Stack
- **Category:** Ethical Hacking / Cyber Security Lab
- **Language:** Python / C (where applicable)
- **Tech Stack:** `Python 3`, `Scapy`, `colorama`, `random`
- **Prerequisites:** `scapy`, `colorama`

---

## 🚀 Key Features
- Interactive menus to select target protocol (TCP, UDP, ICMP).
- Fuzzing presets: Random TCP flags, payload fuzzing, or port sweeps.
- Uses Colorama to format execution logs.
- Validates Scapy library presence before starting.
...
> **IMPORTANT WARNING:** This tool is created strictly for educational, defensive, and authorized security auditing purposes. Fuzzing systems with malformed packets can trigger application crashes. Run this tool only against test endpoints. The developer (**Abdul Salam**) assumes no liability for misuse, damage, or legal consequences resulting from the deployment of this tool.
